...
static Mat4f getTransformationToBox(Box3f box)
{
  ...
  float _mat[16]={
    static_cast<float>(size.x?(0.5*size.x/cos_pi_4):1.0f) , static_cast<float>0                                  , static_cast<float>0                                 , static_cast<float>(mid.x),  
    static_cast<float>0                                   , static_cast<float>(size.y?(0.5*size.y/cos_pi_4):1.0f), static_cast<float>0                                 , static_cast<float>(mid.y), 
    static_cast<float>0                                   , static_cast<float>0                                  , static_cast<float>(size.z?0.5*size.z/cos_pi_4:1.0f) , static_cast<float>(mid.z), 
...
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
void Manipulator::onMouse(int type,int button,int x,int y,Ray3f ray)
{           
  if (!box.isValid() || !T)
    return;
  ...
  bool bFindNewRef= (type==MousePressed) || (type==MouseMoved && !this->bRunning);
  ...
  // from now on...
  this->bRunning= (type==MouseMoved && this->bRunning) || (type==MousePressed && this->ref!=INVALID);
} 


//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
void Manipulator::render(GLCanvas* glcanvas)
{     
  ...
  glcanvas->setPolygonMode(Batch::LINES);
  glcanvas->setLineWidth(3);

  // rotate
  if (this->operation==ROTATE)
  {
    if (!freeze_y && !freeze_z) {if (this->ref==REF_X) circle_x->setColor(Color4f::Yellow()); else circle_x->setColor(Color4f::Red  ());glcanvas->renderBatch(circle_x);}
    if (!freeze_x && !freeze_z) {if (this->ref==REF_Y) circle_y->setColor(Color4f::Yellow()); else circle_y->setColor(Color4f::Green());glcanvas->renderBatch(circle_y);}
    if (!freeze_x && !freeze_y) {if (this->ref==REF_Z) circle_z->setColor(Color4f::Yellow()); else circle_z->setColor(Color4f::Blue ());glcanvas->renderBatch(circle_z);}
  }    

  // translateX
  else if (this->operation==TRANSLATE)
  {
    if (!freeze_x) {if (this->ref==REF_X) line_x->setColor(Color4f::Yellow()); else line_x->setColor(Color4f::Red  ());glcanvas->renderBatch(line_x);}
    if (!freeze_y) {if (this->ref==REF_Y) line_y->setColor(Color4f::Yellow()); else line_y->setColor(Color4f::Green());glcanvas->renderBatch(line_y);}
    if (!freeze_z) {if (this->ref==REF_Z) line_z->setColor(Color4f::Yellow()); else line_z->setColor(Color4f::Blue ());glcanvas->renderBatch(line_z);}
  }  
  ...
  glcanvas->setPolygonMode(Batch::POLYGON);
}
